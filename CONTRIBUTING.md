...
- **Working Branch:** Development occurs in a single long-lived agent branch whose name is auto-generated by Jules and starts with `jules-` (e.g. `jules-3805668974257506057-99790dda`).
- **Archival Snapshot Branches:** The human maintainer periodically snapshots successful milestone states into separate archival branches named `iteration-1`, `iteration-2`, `iteration-3`, etc. (manual, outside of CI, no automation).
- **Minimal `main` Branch:** The `main` branch is intentionally kept minimal (containing foundational documentation like `docs/`). Work does not merge into `main` automatically.
...
## 4. PR Branch-Name Check Policy

Branch-name-check runs exclusively on `pull_request` events and accepts source branch names matching:
- `iteration-<N>` and `iteration-<N>-hotfix` (archival PRs),
- `jules-<id>` (agent PRs).
- Anything else (e.g. `feat/foo`, `main`, `dev`) must be renamed before opening a PR.
