...
def test_user_crud_and_password_reset(admin_client, db_session):
    ...
    reset_resp = admin_client.post(
        ...
        data={"new_password": "newpassword999"},
        follow_redirects=True
    )
    ...
    # Audit log check
    audit = db_session.query(AuditLog).filter(AuditLog.action == "reset_password", AuditLog.entity_id == created_user.id).first()
...
def test_excel_import_hard_error_blocking(admin_client):
    ...
    response = admin_client.post(
        ...
        files={"file": ("invalid_fleet.xlsx", file_stream.getvalue(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")},
    )
