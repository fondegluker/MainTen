...
def login_page(request: Request, error: str = None, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "login.html",
        context_with_defaults(request, current_user, {"error": error})
    )
...
def admin_dashboard(
    ...
    scheduled_events = db.query(MaintenanceEvent).filter(MaintenanceEvent.status == MaintenanceEventStatus.PLANNED).count()
    ...
    return templates.TemplateResponse(
        ...
        context_with_defaults(request, current_user, {"stats": stats})
    )
...
def user_computers(
    ...
    return templates.TemplateResponse(
        "user_computers.html",
        context_with_defaults(request, current_user, {"computers": computers})
    )
