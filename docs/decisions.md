...
- **Working Branch Model:** Development occurs in a single long-lived agent branch whose name is auto-generated by Jules and starts with `jules-` (e.g. `jules-3805668974257506057-99790dda`).
...
- **Minimal `main` Branch:** `main` is intentionally kept minimal (containing foundational documentation like `docs/`). Work does not merge into `main` automatically.
- **PR Branch-Name Policy:** Branch-name-check applies ONLY to PRs on `pull_request` events and accepts:
  - `iteration-<N>` and `iteration-<N>-hotfix` (archival PRs),
  - `jules-<id>` (agent PRs).
  - Anything else must be renamed before opening a PR.
  - The `skip-branch-name-check` label is available as an admin escape hatch and is logged.
